...
.auth-box::before{content:'';position:absolute;inset:-1px;background:linear-gradient(135deg,#F5C842,#F0427A,#a855f7,#F5C842);background-size:300% 300%;border-radius:21px;z-index:-1;animation:aurora 4s linear infinite}
...
.nav-item.active{background:linear-gradient(135deg,rgba(245,200,66,.12),rgba(240,66,122,.08));color:var(--text);border-left:3px solid var(--yellow)}
...
.modal{background:#111;border:1px solid rgba(245,200,66,.15);border-radius:18px;padding:30px;width:460px;max-width:95vw;transform:scale(.93);transition:transform .3s;position:relative;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.6);max-height:90vh;overflow-y:auto}
...
<div id="landing" style="display:none;min-height:100vh;background:var(--bg);position:relative;z-index:1;overflow:hidden">
  <!-- Bg glow -->
  <div style="position:absolute;top:-200px;left:50%;transform:translateX(-50%);width:800px;height:600px;background:radial-gradient(ellipse,rgba(245,200,66,.07) 0%,transparent 70%);pointer-events:none"></div>
  <div style="position:absolute;bottom:-100px;right:-100px;width:500px;height:500px;background:radial-gradient(ellipse,rgba(240,66,122,.08) 0%,transparent 70%);pointer-events:none"></div>

  <!-- Nav -->
  <nav style="padding:18px 40px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;background:rgba(10,10,10,.85);backdrop-filter:blur(20px);border-bottom:1px solid rgba(255,255,255,.06);z-index:10">
    <div style="display:flex;align-items:center;gap:10px">
      <div class="logo-icon" style="width:34px;height:34px;font-size:16px"><img id="land-logo-img" style="width:100%;height:100%;object-fit:contain;display:none"><span id="land-logo-emoji">🔗</span></div>
      <span style="font-size:17px;font-weight:800">AffiHub</span>
    </div>
    <button class="btn btn-gst btn-sm" onclick="showAuth()" style="font-size:12px">Connexion</button>
  </nav>

  <!-- Hero -->
  <div style="max-width:860px;margin:0 auto;padding:90px 24px 60px;text-align:center;position:relative">
    <div style="display:inline-flex;align-items:center;gap:8px;background:rgba(245,200,66,.08);border:1px solid rgba(245,200,66,.18);border-radius:20px;padding:6px 18px;font-size:12px;font-weight:700;color:var(--yellow);margin-bottom:28px;letter-spacing:.5px">⚡ Accès sur invitation</div>
    <h1 style="font-size:clamp(40px,7vw,72px);font-weight:900;line-height:1.05;letter-spacing:-2px;margin-bottom:22px;color:var(--text)">Transforme ton<br><span style="background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-size:200% 200%;animation:gradient-shift 4s ease infinite">audience en revenus</span></h1>
    <p style="font-size:17px;color:var(--muted);max-width:480px;margin:0 auto 44px;line-height:1.75">Rejoins notre réseau d'affiliation privé. Génère tes liens, suis tes gains en temps réel et retire en crypto.</p>
    <button class="btn btn-grad" onclick="showAuthRegister()" style="font-size:16px;padding:16px 40px;border-radius:14px;box-shadow:0 8px 32px rgba(240,66,122,.4)">🚀 Commencer l'aventure</button>
    <div style="margin-top:16px;font-size:12px;color:var(--muted)">Déjà inscrit ? <button onclick="showAuth()" style="background:none;border:none;color:var(--yellow);cursor:pointer;font-family:inherit;font-size:12px;font-weight:700">Se connecter</button></div>
  </div>

  <!-- Stats bar -->
  <div style="max-width:700px;margin:0 auto 70px;padding:0 24px">
    <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:16px;padding:20px 32px;display:grid;grid-template-columns:repeat(3,1fr);gap:12px;text-align:center">
      <div><div style="font-size:26px;font-weight:900;background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent">$25</div><div style="font-size:11px;color:var(--muted);margin-top:3px">Retrait minimum</div></div>
      <div style="border-left:1px solid rgba(255,255,255,.08);border-right:1px solid rgba(255,255,255,.08)"><div style="font-size:26px;font-weight:900;color:var(--green)">5%</div><div style="font-size:11px;color:var(--muted);margin-top:3px">Commission parrainage</div></div>
      <div><div style="font-size:26px;font-weight:900;color:var(--yellow)">3</div><div style="font-size:11px;color:var(--muted);margin-top:3px">Cryptos acceptées</div></div>
    </div>
  </div>

  <!-- Features -->
  <div style="max-width:860px;margin:0 auto 70px;padding:0 24px">
    <div style="text-align:center;margin-bottom:36px"><h2 style="font-size:28px;font-weight:900;margin-bottom:8px">Tout ce dont tu as besoin</h2><p style="color:var(--muted);font-size:14px">Un dashboard complet pour gérer ton activité d'affiliation</p></div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px">
      <div style="background:rgba(245,200,66,.05);border:1px solid rgba(245,200,66,.15);border-radius:16px;padding:22px"><div style="font-size:30px;margin-bottom:12px">🔗</div><div style="font-weight:800;margin-bottom:6px;font-size:15px">Liens trackés</div><div style="font-size:13px;color:var(--muted);line-height:1.6">Chaque clic et conversion enregistrés automatiquement en temps réel</div></div>
      <div style="background:rgba(0,214,143,.05);border:1px solid rgba(0,214,143,.15);border-radius:16px;padding:22px"><div style="font-size:30px;margin-bottom:12px">💰</div><div style="font-weight:800;margin-bottom:6px;font-size:15px">Paiements crypto</div><div style="font-size:13px;color:var(--muted);line-height:1.6">Retire tes gains en Bitcoin, Solana ou Litecoin dès $25</div></div>
      <div style="background:rgba(77,158,255,.05);border:1px solid rgba(77,158,255,.15);border-radius:16px;padding:22px"><div style="font-size:30px;margin-bottom:12px">📊</div><div style="font-weight:800;margin-bottom:6px;font-size:15px">Stats complètes</div><div style="font-size:13px;color:var(--muted);line-height:1.6">Clics, conversions, gains et taux par offre — tout en un coup d'œil</div></div>
      <div style="background:rgba(240,66,122,.05);border:1px solid rgba(240,66,122,.15);border-radius:16px;padding:22px"><div style="font-size:30px;margin-bottom:12px">👥</div><div style="font-weight:800;margin-bottom:6px;font-size:15px">Parrainage 5%</div><div style="font-size:13px;color:var(--muted);line-height:1.6">Invite tes amis et gagne 5% sur toutes leurs conversions approuvées</div></div>
      <div style="background:rgba(168,85,247,.05);border:1px solid rgba(168,85,247,.15);border-radius:16px;padding:22px"><div style="font-size:30px;margin-bottom:12px">🎨</div><div style="font-weight:800;margin-bottom:6px;font-size:15px">Liens personnalisés</div><div style="font-size:13px;color:var(--muted);line-height:1.6">Personnalise ta page avec ton branding — logo, photos, slogan</div></div>
      <div style="background:rgba(136,136,136,.05);border:1px solid rgba(136,136,136,.15);border-radius:16px;padding:22px"><div style="font-size:30px;margin-bottom:12px">🎫</div><div style="font-weight:800;margin-bottom:6px;font-size:15px">Support dédié</div><div style="font-size:13px;color:var(--muted);line-height:1.6">Un manager disponible sur Discord pour t'accompagner</div></div>
    </div>
  </div>

  <!-- CTA final -->
  <div style="max-width:600px;margin:0 auto;padding:0 24px 80px;text-align:center">
    <div style="background:var(--grad);border-radius:20px;padding:40px 32px;background-size:200% 200%;animation:gradient-shift 4s ease infinite">
      <h2 style="font-size:26px;font-weight:900;color:#000;margin-bottom:10px">Prêt à commencer ?</h2>
      <p style="font-size:14px;color:rgba(0,0,0,.65);margin-bottom:24px">Crée ton compte en 30 secondes et commence à générer tes premiers gains</p>
      <button class="btn" onclick="showAuthRegister()" style="background:rgba(0,0,0,.15);color:#000;border:2px solid rgba(0,0,0,.2);font-size:15px;padding:14px 36px;border-radius:12px;font-weight:800">🚀 Créer mon compte</button>
    </div>
  </div>
</div>
<div id="auth-screen">
  <div class="auth-box">
    ...
    <div id="register-form" style="display:none">
      <div class="fg"><label>Nom complet</label><input type="text" id="r-name" placeholder="Votre nom"></div>
...
async function api(method,path,body){
  ...
  if(!r.ok)throw new Error(data.error||'Erreur');return data;
}
...
async function doLogin(){
  ...
  TOKEN=r.token;ME=r.user;localStorage.setItem('affihub_token',TOKEN);localStorage.setItem('affihub_user',JSON.stringify(ME));startApp();}
  catch(e){toast(e.message,'e');}
}

async function doRegister(){
  try{const r=await api('POST','/api/register',{name:document.getElementById('r-name').value.trim(),email:document.getElementById('r-email').value.trim(),password:document.getElementById('r-pass').value.trim(),referral_code:document.getElementById('r-ref').value.trim()});
  TOKEN=r.token;ME=r.user;localStorage.setItem('affihub_token',TOKEN);localStorage.setItem('affihub_user',JSON.stringify(ME));startApp();toast('Bienvenue !','s');}
  catch(e){toast(e.message,'e');}
}
...
function startApp(){
  ...
  const adminNav=[
    ...
    {section:'GESTION',items:[{icon:'👥',label:'Affiliés',page:'admin-aff'},{icon:'🔁',label:'Conversions',page:'admin-conv'},{icon:'💸',label:'Retraits',page:'admin-wd'},{icon:'🤝',label:'Parrainages',page:'admin-ref'},{icon:'🎫',label:'Tickets',page:'admin-tickets'},{icon:'🎨',label:'Liens perso',page:'admin-custom'}]},
    {section:'CLASSEMENT',items:[{icon:'🏆',label:'TOP Affiliés',page:'aff-top'}]},
    {section:'CONFIGURATION',items:[{icon:'🎯',label:'Offres',page:'admin-offers'},{icon:'🔗',label:'Liens',page:'admin-links'},{icon:'⚙️',label:'Paramètres',page:'admin-settings'}]}
  ];
  const affNav=[
    ...
    {section:'GAINS',items:[{icon:'💸',label:'Paiements',page:'aff-pay'},{icon:'👥',label:'Parrainage',page:'aff-ref'}]},
    {section:'CLASSEMENT',items:[{icon:'🏆',label:'TOP Affiliés',page:'aff-top'}]},
    {section:'SUPPORT',items:[{icon:'👨‍💼',label:'Mon manager',page:'aff-support'},{icon:'🎫',label:'Ticket / Aide',page:'aff-tickets'},{icon:'⚙️',label:'Paramètres',page:'aff-settings'}]}
  ];
  if(ME.role==='admin'){
    document.getElementById('role-badge').textContent='👑 Admin';
    buildNav(adminNav);buildMobileNav(adminNav);goPage('admin-dash');
  }else{
    document.getElementById('role-badge').textContent='⚡ Affilié';
    buildNav(affNav);buildMobileNav(affNav);goPage('aff-dash');
  }
  clearInterval(refreshTimer);refreshTimer=setInterval(()=>{if(curPage)goPage(curPage);refreshNotifs();},30000);setTimeout(refreshNotifs,500);
}

function logout(){TOKEN=null;ME=null;localStorage.removeItem('affihub_token');localStorage.removeItem('affihub_user');clearInterval(refreshTimer);document.getElementById('app').style.display='none';document.getElementById('auth-screen').style.display='flex';}
...
const TITLES={
  ...
  'aff-tickets':'🎫 Ticket / Aide','aff-settings':'Paramètres',
  'admin-dash':'Dashboard','admin-aff':'Affiliés','admin-conv':'Conversions','admin-wd':'Retraits',
  'admin-ref':'Parrainages','admin-tickets':'🎫 Tickets','admin-custom':'🎨 Liens personnalisés',
  'admin-offers':'Offres','admin-links':'Liens','admin-settings':'Paramètres du site'
};
...
async function goPage(page){
  ...
  try{const html=await renderPage(page);document.getElementById('content').innerHTML=html;}
  catch(e){document.getElementById('content').innerHTML='<div class="empty"><div class="empty-icon">⚠️</div><p>'+e.message+'</p></div>';}
}
...
async function uploadImg(file,bucket){
  ...
  const r=await fetch(SURL+'/storage/v1/object/'+bucket+'/'+fileName,{method:'POST',headers:{'Authorization':'Bearer '+SKEY,'Content-Type':file.type,'x-upsert':'true'},body:file});
...
async function renderPage(page){
    ...
    CATS.forEach(cat=>{
      ...
      catOffers.forEach(o=>{
        ...
        if(el){
          ...
          bottomHtml='<div style="background:rgba(0,0,0,.3);border:1px solid #2a2a2a;border-radius:9px;padding:8px 12px;margin-bottom:12px;display:flex;align-items:center;gap:8px"><code style="flex:1;font-family:monospace;font-size:11px;color:'+(el.active?cat.color:'var(--muted)')+';overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+linkUrl+'</code><button class="copy-btn" onclick="cpLink(\''+el.id+'\',this)">📋</button></div><div style="display:flex;align-items:center;justify-content:space-between"><div style="text-align:center"><div style="font-size:16px;font-weight:800">'+el.clicks+'</div><div style="font-size:10px;color:var(--muted)">CLICS</div></div><div style="display:flex;gap:6px">'+statusBadge+regenBtn+'</div></div>';
        } else if(cat.key==='autre'){
          const cr=customReqs.find(r=>r.offer_id===o.id);
          if(cr&&cr.custom_link){
            bottomHtml='<div style="background:rgba(0,214,143,.08);border:1px solid rgba(0,214,143,.2);border-radius:9px;padding:10px 14px;margin-bottom:10px;display:flex;align-items:center;gap:8px"><code style="flex:1;font-size:11px;color:var(--green);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+cr.custom_link+'</code><button class="copy-btn" onclick="navigator.clipboard?.writeText(\''+cr.custom_link+'\');toast(\'Lien copié !\',\'s\')">📋</button></div><span class="badge bg2">✅ Lien reçu</span>';
          } else if(cr){
            bottomHtml='<span class="badge by2">⏳ Demande en cours</span><br><button class="btn btn-gst btn-sm" onclick="openCustomRequest('+o.id+',\''+o.name.replace(/'/g,"\\'")+'\')" style="margin-top:8px;width:100%;justify-content:center">✏️ Modifier</button>';
          } else {
            bottomHtml='<button class="btn btn-grad" onclick="openCustomRequest('+o.id+',\''+o.name.replace(/'/g,"\\'")+'\')\" style="width:100%;justify-content:center">🎨 Personnaliser & Demander</button>';
          }
        } else {
        ...
        html+='<div style="background:#111;border:1px solid '+cat.border+';border-radius:14px;overflow:hidden;position:relative"><div style="position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,'+cat.color+',transparent);z-index:1"></div>'+imgHtml+'<div style="padding:18px"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:14px"><div><div style="font-size:15px;font-weight:700;margin-bottom:4px">'+o.name+'</div><div style="font-size:12px;color:var(--muted)">'+(o.description||'')+'</div></div><div style="background:'+cat.bg+';border:1px solid '+cat.border+';border-radius:10px;padding:6px 12px;text-align:center;flex-shrink:0"><div style="font-size:18px;font-weight:900;color:'+cat.color+'">$'+o.commission+'</div><div style="font-size:10px;color:var(--muted)">/ vente</div></div></div>'+bottomHtml+'</div></div>';
      });
      html+='</div></div></div>';
    });
    return html;
  ...
  if(page==='aff-stats'){
    ...
    const rows=links.map(l=>{
      const approved=convs.filter(c=>c.link_id===l.id&&c.status==='approved');
      const pending=convs.filter(c=>c.link_id===l.id&&c.status==='pending');
      const gains=approved.reduce((s,c)=>s+c.amount,0);
      const offerName=offers.find(o=>o.id===l.offer_id)?.name||'?';
      return '<tr><td><div style="font-weight:700;margin-bottom:4px">'+offerName+'</div><code style="font-family:monospace;font-size:11px;color:var(--muted)">'+origin+'/go/'+l.id+'</code></td><td style="text-align:center;font-weight:800;font-size:16px">'+l.clicks+'</td><td style="text-align:center"><span style="color:var(--green);font-weight:800">'+approved.length+'</span>'+(pending.length>0?' <span style="color:var(--yellow);font-size:11px">(+'+pending.length+')</span>':'')+'</td><td style="text-align:center;font-weight:900;background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-size:15px">$'+gains+'</td><td>'+(l.active?'<span class="badge bg2">✓ Actif</span>':'<span class="badge br2">Désactivé</span>')+'</td></tr>';
    }).join('');
    return '<div class="card"><div class="card-title" style="margin-bottom:18px">📈 Performance de vos liens</div><div class="tw"><table><thead><tr><th>🎯 Offre / Lien</th><th style="text-align:center">🖱️ Clics</th><th style="text-align:center">✅ Conversions</th><th style="text-align:center">💰 Gains</th><th>📌 Statut</th></tr></thead><tbody>'+rows+'</tbody></table></div></div>';
  }
  ...
  if(page==='aff-conv'){
    ...
    const tabHtml=tabs.map(([f,l,n])=>'<button class="tab'+(f==='all'?' active':'\"')+'" onclick="filterConv(this,\''+f+'\','+JSON.stringify(convs).replace(/'/g,"\\'")+')">'+l+' ('+n+')</button>').join('');
  ...
  if(page==='aff-pay'){
    ...
    +'<div class="fi"><label>Montant ($)</label><div style="display:flex;gap:8px"><input type="number" id="wd-amt" min="25" placeholder="Minimum $25" style="flex:1"><button class="btn btn-grad btn-sm" onclick="document.getElementById(\'wd-amt\').value='+bal+'">MAX $'+bal+'</button></div></div>'
    +'<div class="fi"><label>Crypto</label><div class="crypto-grid">'
    +'<div class="crypto-opt" onclick="pickCrypto(\'BTC\',this)"><svg width="32" height="32" viewBox="0 0 32 32" style="display:block;margin:0 auto 6px"><circle cx="16" cy="16" r="16" fill="#F7931A"/><path d="M22.2 13.8c.3-2-1.2-3.1-3.3-3.8l.7-2.7-1.6-.4-.6 2.6-1.3-.3.6-2.6-1.6-.4-.7 2.7-1-.3v-.1l-2.2-.5-.4 1.7s1.2.3 1.1.3c.6.2.7.6.7.9l-1.7 6.8c-.1.2-.3.5-.8.4 0 .1-1.1-.3-1.1-.3l-.8 1.8 2.1.5 1.1.3-.7 2.7 1.6.4.7-2.7 1.3.3-.7 2.7 1.6.4.7-2.7c2.7.5 4.7.3 5.5-2.1.7-1.9-.1-3-1.4-3.7 1-.2 1.7-.9 1.9-2.3zm-3.4 4.8c-.5 2-3.8.9-4.9.6l.9-3.5c1.1.3 4.5.8 4 2.9zm.5-4.8c-.4 1.8-3.2.9-4.1.7l.8-3.2c.9.2 3.8.6 3.3 2.5z" fill="white"/></svg>Bitcoin</div>'
    +'<div class="crypto-opt" onclick="pickCrypto(\'SOL\',this)"><svg width="32" height="32" viewBox="0 0 32 32" style="display:block;margin:0 auto 6px"><circle cx="16" cy="16" r="16" fill="url(#solGrad)"/><defs><linearGradient id="solGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style="stop-color:#9945FF"/><stop offset="100%" style="stop-color:#14F195"/></linearGradient></defs><path d="M10 21.5h12.5l-2 1.8H8L10 21.5zm0-6h12.5l-2 1.8H8L10 15.5zm2-6h12.5l-2 1.8H10L12 9.5z" fill="white"/></svg>Solana</div>'
    +'<div class="crypto-opt" onclick="pickCrypto(\'LTC\',this)"><svg width="32" height="32" viewBox="0 0 32 32" style="display:block;margin:0 auto 6px"><circle cx="16" cy="16" r="16" fill="#BFBBBB"/><text x="16" y="23" text-anchor="middle" font-size="18" font-weight="bold" fill="white">Ł</text></svg>Litecoin</div>'
    +'</div></div>'
    +'<div class="fi"><label>Adresse crypto</label><input type="text" id="wd-addr" placeholder="Votre adresse de réception"></div>'
    +'<button class="btn btn-grad" onclick="doWd()" style="width:100%;justify-content:center">💸 Soumettre</button></div>'
    +'<div class="card" style="margin-bottom:0;border-color:rgba(245,200,66,.2)"><div style="font-size:13px;font-weight:800;color:var(--yellow);margin-bottom:14px">💰 Barème des frais</div><table style="width:100%;font-size:12px;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:8px;color:var(--muted);font-size:10px;border-bottom:1px solid #222">Montant</th><th style="text-align:right;padding:8px;color:var(--muted);font-size:10px;border-bottom:1px solid #222">Frais</th></tr></thead><tbody><tr style="border-bottom:1px solid #1a1a1a"><td style="padding:10px">25–49 $</td><td style="padding:10px;text-align:right;color:var(--red);font-weight:800">5%</td></tr><tr style="border-bottom:1px solid #1a1a1a"><td style="padding:10px">50–99 $</td><td style="padding:10px;text-align:right;color:var(--yellow);font-weight:800">4%</td></tr><tr style="border-bottom:1px solid #1a1a1a"><td style="padding:10px">100–199 $</td><td style="padding:10px;text-align:right;color:var(--yellow);font-weight:800">3%</td></tr><tr><td style="padding:10px">200 $+</td><td style="padding:10px;text-align:right;color:var(--green);font-weight:800">2%</td></tr></tbody></table></div></div>'
    +'<div class="card"><div class="card-title" style="margin-bottom:16px">📋 Historique</div>'+(wds.length===0?'<div class="empty"><div class="empty-icon">💳</div><p>Aucun retrait</p></div>':'<div class="tw"><table><thead><tr><th>📅 Date</th><th>💰 Montant</th><th>🪙 Crypto</th><th>📬 Adresse</th><th>📌 Statut</th></tr></thead><tbody>'+wrows+'</tbody></table></div>')+'</div>';
  }
  ...
  if(page==='aff-top'){
    ...
    const mkRows=sort=>{
      ...
      return sorted.map((u,i)=>'<tr><td style="text-align:center;font-size:18px">'+getMedal(i)+'</td><td><div style="display:flex;align-items:center;gap:8px"><div class="avatar" style="width:28px;height:28px;font-size:12px">'+u.name[0]+'</div><b>'+u.name+'</b></div></td><td style="color:var(--muted)">'+fmt(u.created_at)+'</td><td style="color:#4D9EFF;font-weight:800;text-align:center">'+(u.totalClicks||0)+'</td><td style="color:var(--yellow);font-weight:800;text-align:center">'+u.totalConversions+'</td><td style="color:var(--green);font-weight:800;text-align:center">$'+u.totalGains+'</td></tr>').join('');
    };
  ...
  if(page.startsWith('aff-ticket-')){
    ...
    const reasons={'question':'❓ Question','bug':'🐛 Bug','payement':'💸 Paiement','compte':'👤 Compte','offre':'🎯 Offre','mes-liens':'🔗 Mes liens','suggestion':'💡 Suggestion'};
    const msgs=ticket.messages.map(m=>{const isMe=m.user_id===ME.id;const isAdmin=m.users?.role==='admin';return '<div style="display:flex;flex-direction:column;align-items:'+(isMe?'flex-end':'flex-start')+';margin-bottom:16px"><div style="font-size:11px;color:var(--muted);margin-bottom:4px">'+(isAdmin?'👑 Admin':'👤 '+m.users?.name)+' • '+fmt(m.created_at)+'</div><div style="background:'+(isMe?'var(--grad)':'var(--bg3)')+';color:'+(isMe?'#000':'var(--text)')+';border-radius:'+(isMe?'14px 14px 4px 14px':'14px 14px 14px 4px')+';padding:12px 16px;max-width:70%">'+(m.content?'<div style="font-size:13px;line-height:1.6">'+m.content+'</div>':'')+(m.image_url?'<img src="'+m.image_url+'" style="max-width:100%;max-height:200px;border-radius:8px;margin-top:'+(m.content?'8px':'0')+';object-fit:cover;cursor:pointer" onclick="window.open(\''+m.image_url+'\',\'_blank\')">':'')+'</div></div>';}).join('');
    ...
    +(ticket.status==='open'?'<div class="card"><div class="fi"><label>Votre réponse</label><textarea id="reply-msg" placeholder="Écrivez votre message..." style="resize:vertical;min-height:80px"></textarea></div><div class="fi"><label>Image (optionnel)</label><div style="border:2px dashed #333;border-radius:10px;padding:16px;text-align:center;cursor:pointer" onclick="document.getElementById(\'reply-img-file\').click()"><input type="file" id="reply-img-file" accept="image/*" style="display:none" onchange="previewReplyImg(this)"><div id="reply-img-prev" style="display:none;margin-bottom:8px"><img id="reply-img-prev-img" style="max-height:120px;border-radius:8px"></div><div id="reply-img-ph"><span style="font-size:20px">📎</span><div style="font-size:12px;color:var(--muted);margin-top:4px">Cliquer pour ajouter une image</div></div></div></div><button class="btn btn-grad" onclick="sendReply('+ticket.id+')" style="width:100%;justify-content:center">📨 Envoyer</button></div>':'<div class="card" style="text-align:center;padding:20px"><div style="font-size:28px;margin-bottom:8px">'+(ticket.status==='resolved'?'✅':'🔴')+'</div><div style="font-weight:700">'+(ticket.status==='resolved'?'Ticket résolu':'Ticket fermé')+'</div></div>');
  }
  ...
  if(page==='admin-aff'){
    ...
    const rows=users.map(u=>'<tr><td><div style="display:flex;align-items:center;gap:9px"><div class="avatar" style="width:30px;height:30px;font-size:12px">'+u.name[0]+'</div><b>'+u.name+'</b></div></td><td style="color:var(--muted)">'+u.email+'</td><td>'+gtext('$'+(u.balance||0))+'</td><td style="color:var(--muted)">'+fmt(u.created_at)+'</td><td><button class="btn btn-red btn-sm" onclick="confirmDeleteAff(this)" data-id="'+u.id+'" data-name="'+u.name.replace(/"/g,'&quot;')+'" data-email="'+u.email+'">🗑 Supprimer</button></td></tr>').join('');
    return '<div class="card"><div class="card-header"><div class="card-title">Affiliés</div><span class="badge bp2">'+users.length+' affilié(s)</span></div><div class="tw"><table><thead><tr><th>👤 Affilié</th><th>📧 Email</th><th>💰 Solde</th><th>📅 Inscrit</th><th>⚙️</th></tr></thead><tbody>'+rows+'</tbody></table></div></div>';
  }
  ...
  if(page==='admin-wd'){
    ...
    return '<div class="card"><div class="card-title" style="margin-bottom:16px">Retraits</div>'+(rows?'<div class="tw"><table><thead><tr><th>📅 Date</th><th>👤 Affilié</th><th>💰 Montant</th><th>🪙 Crypto</th><th>📬 Adresse</th><th>📌 Statut</th><th>⚙️</th></tr></thead><tbody>'+rows+'</tbody></table></div>':'<div class="empty"><div class="empty-icon">💸</div><p>Aucune demande</p></div>')+'</div>';
  }

  if(page==='admin-ref'){
    const affiliates=await api('GET','/api/admin/referrals');
    if(affiliates.length===0)return '<div class="card"><div class="empty"><div class="empty-icon">🤝</div><p>Aucun parrainage</p></div></div>';
    const cards=affiliates.map(aff=>'<div class="card" style="cursor:pointer;margin-bottom:12px" onclick="goPage(\'admin-ref-'+aff.id+'\')"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px"><div style="display:flex;align-items:center;gap:12px"><div class="avatar" style="width:44px;height:44px;font-size:18px">'+aff.name[0]+'</div><div><div style="font-size:15px;font-weight:700">'+aff.name+'</div><div style="font-size:12px;color:var(--muted)">'+aff.email+'</div></div></div><div style="display:flex;gap:20px;text-align:center"><div><div style="font-size:20px;font-weight:800;color:var(--yellow)">'+aff.filleules.length+'</div><div style="font-size:10px;color:var(--muted)">Filleuls</div></div><div><div style="font-size:20px;font-weight:800;color:var(--green)">$'+aff.totalEarned.toFixed(2)+'</div><div style="font-size:10px;color:var(--muted)">Commissions</div></div><span style="color:var(--muted);font-size:18px;align-self:center">→</span></div></div></div>').join('');
    return '<div style="margin-bottom:20px;font-size:13px;color:var(--muted)">Cliquez pour voir le détail</div>'+cards;
  }
  ...
  if(page.startsWith('admin-ref-')){
    ...
    const fRows=aff.filleules.map(f=>{const isActive=f.referral_active!==false;const earned=aff.commissions.filter(c=>c.referee_id===f.id).reduce((s,c)=>s+c.amount,0);return '<tr><td><div style="display:flex;align-items:center;gap:8px"><div class="avatar" style="width:26px;height:26px;font-size:11px">'+f.name[0]+'</div><b>'+f.name+'</b></div></td><td style="color:var(--muted)">'+fmt(f.created_at)+'</td><td style="color:var(--green)">$'+earned.toFixed(2)+'</td><td>'+(isActive?'<span class="badge bg2">✅ Actif</span>':'<span class="badge br2">🚫 Arrêté</span>')+'</td><td>'+(isActive?'<button class="btn btn-red btn-sm" onclick="toggleRef(\''+f.id+'\',false,\''+affId+'\')">🚫 Arrêter</button>':'<button class="btn btn-grn btn-sm" onclick="toggleRef(\''+f.id+'\',true,\''+affId+'\')">✅ Réactiver</button>')+'</td></tr>';}).join('');
    ...
    +'<div class="card"><div class="card-title" style="margin-bottom:16px">👥 Filleuls</div>'+(aff.filleules.length===0?'<div class="empty"><p>Aucun filleul</p></div>':'<div class="tw"><table><thead><tr><th>👤 Filleul</th><th>📅 Date</th><th>💰 Gains</th><th>📌 Statut</th><th>⚙️</th></tr></thead><tbody>'+fRows+'</tbody></table></div>')+'</div>'
  ...
  if(page.startsWith('admin-ticket-')){
    ...
    const reasons={'question':'❓ Question','bug':'🐛 Bug','payement':'💸 Paiement','compte':'👤 Compte','offre':'🎯 Offre','mes-liens':'🔗 Mes liens','suggestion':'💡 Suggestion'};
    const msgs=ticket.messages.map(m=>{const isAdmin=m.users?.role==='admin';return '<div style="display:flex;flex-direction:column;align-items:'+(isAdmin?'flex-end':'flex-start')+';margin-bottom:16px"><div style="font-size:11px;color:var(--muted);margin-bottom:4px">'+(isAdmin?'👑 Admin':'👤 '+m.users?.name)+' • '+fmt(m.created_at)+'</div><div style="background:'+(isAdmin?'var(--grad)':'var(--bg3)')+';color:'+(isAdmin?'#000':'var(--text)')+';border-radius:'+(isAdmin?'14px 14px 4px 14px':'14px 14px 14px 4px')+';padding:12px 16px;max-width:70%">'+(m.content?'<div style="font-size:13px;line-height:1.6">'+m.content+'</div>':'')+(m.image_url?'<img src="'+m.image_url+'" style="max-width:100%;max-height:200px;border-radius:8px;cursor:pointer" onclick="window.open(\''+m.image_url+'\',\'_blank\')">':'')+'</div></div>';}).join('');
  ...
  if(page==='admin-custom'){
    ...
    const rows=requests.map(r=>{
      const photoHtml=[r.photo1_url,r.photo2_url,r.photo3_url,r.photo4_url,r.photo5_url,r.photo6_url].filter(Boolean).map(p=>'<img src="'+p+'" style="width:80px;height:80px;border-radius:8px;object-fit:cover;cursor:pointer;'+(r.photos_blurred?'filter:blur(4px)':'')+'" onclick="window.open(\''+p+'\',\'_blank\')">').join('');
      const tags=[r.tag1,r.tag2,r.tag3].filter(Boolean).map(t=>'<span class="badge bp2">'+t+'</span>').join('');
      const linkSection=r.status==='approved'
        ?'<div style="background:rgba(0,214,143,.08);border:1px solid rgba(0,214,143,.2);border-radius:10px;padding:10px 14px;display:flex;align-items:center;justify-content:space-between"><code style="font-size:12px;color:var(--green)">'+r.custom_link+'</code><button class="btn btn-red btn-sm" onclick="deleteCustomReq('+r.id+')">🗑</button></div>'
        :'<div style="display:flex;gap:8px;align-items:center"><input type="text" id="clink-'+r.id+'" placeholder="Colle le lien ici..." style="flex:1;background:var(--bg3);border:1px solid #2a2a2a;border-radius:10px;padding:10px 14px;color:var(--text);font-family:inherit;font-size:13px;outline:none"><button class="btn btn-grad" onclick="sendCustomLink('+r.id+')">✅ Envoyer</button><button class="btn btn-red btn-sm" onclick="deleteCustomReq('+r.id+')">🗑</button></div>';
      return '<div class="card" style="margin-bottom:14px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px"><div style="display:flex;align-items:center;gap:12px">'+(r.logo_url?'<img src="'+r.logo_url+'" style="width:48px;height:48px;border-radius:12px;object-fit:cover;border:1px solid #2a2a2a">':'<div style="width:48px;height:48px;border-radius:12px;background:#1a1a1a;display:flex;align-items:center;justify-content:center;font-size:20px">🎮</div>')+'<div><div style="font-size:16px;font-weight:800">'+(r.server_name||'Sans nom')+'</div><div style="font-size:12px;color:var(--muted)">'+(r.users?.name)+' • '+(r.offers?.name)+' • '+fmt(r.created_at)+'</div></div></div>'+(r.status==='approved'?'<span class="badge bg2">✅ Lien envoyé</span>':'<span class="badge by2">⏳ En attente</span>')+'</div>'
      +'<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px;margin-bottom:14px"><div style="background:var(--bg3);border-radius:10px;padding:12px"><div style="font-size:10px;color:var(--yellow);text-transform:uppercase;letter-spacing:.8px;margin-bottom:4px">Slogan</div><div style="font-size:13px">'+(r.slogan||'—')+'</div></div><div style="background:var(--bg3);border-radius:10px;padding:12px"><div style="font-size:10px;color:var(--yellow);text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px">Étiquettes</div><div style="display:flex;gap:4px;flex-wrap:wrap">'+(tags||'—')+'</div></div><div style="background:var(--bg3);border-radius:10px;padding:12px"><div style="font-size:10px;color:var(--yellow);text-transform:uppercase;letter-spacing:.8px;margin-bottom:4px">Salons</div><div style="font-size:13px">'+(r.salons||'—')+'</div></div><div style="background:var(--bg3);border-radius:10px;padding:12px"><div style="font-size:10px;color:var(--yellow);text-transform:uppercase;letter-spacing:.8px;margin-bottom:4px">Texte photos</div><div style="font-size:13px">'+(r.photo_text||'—')+'</div></div><div style="background:var(--bg3);border-radius:10px;padding:12px"><div style="font-size:10px;color:var(--yellow);text-transform:uppercase;letter-spacing:.8px;margin-bottom:4px">Photos floutées</div><div style="font-size:13px">'+(r.photos_blurred?'✅ Oui':'❌ Non')+'</div></div></div>'
      +(photoHtml?'<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">'+photoHtml+'</div>':'')+linkSection+'</div>';
    }).join('');
    return '<div class="card-header" style="margin-bottom:20px"><div class="card-title">🎨 Liens personnalisés</div>'+(pending.length>0?'<span class="badge by2">⏳ '+pending.length+' en attente</span>':'')+'</div>'+rows;
  }
  ...
  if(page==='admin-offers'){
    ...
    CATS.forEach(cat=>{
      ...
      catOffers.forEach(o=>{
        ...
        html+='<div class="offer-card"><div style="display:flex;align-items:center;gap:14px;flex:1">'+(o.image_url?'<img src="'+o.image_url+'" style="width:56px;height:56px;border-radius:10px;object-fit:cover;flex-shrink:0;border:1px solid #2a2a2a" onerror="this.style.display=\'none\'">':`<div style="width:56px;height:56px;border-radius:10px;background:#1a1a1a;display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0">🎯</div>`)+'<div><div style="font-size:14px;font-weight:700;margin-bottom:4px">'+o.name+'</div><div style="font-size:12px;color:var(--muted)">'+(o.description||'')+'</div><div style="font-size:11px;color:var(--muted);margin-top:4px">🔗 '+o.url.substring(0,40)+'...</div></div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px">'+gtext('$'+o.commission)+'<div>'+sbadge(o.active?'active':'disabled')+'</div><div style="display:flex;gap:6px"><button class="btn btn-gst btn-sm" onclick="openEditOffer('+o.id+',\''+n+'\',\''+d+'\',\''+u+'\','+o.commission+',\''+(o.category||'autre')+'\',\''+img+'\')">✏️</button><button class="btn btn-red btn-sm" onclick="confirmDeleteOffer('+o.id+',\''+n+'\')">🗑</button></div></div></div>';
      });
      html+='</div></div>';
    });
    return html;
...
function pickCrypto(c,el){selCrypto=c;document.querySelectorAll('.crypto-opt').forEach(e=>e.classList.remove('sel'));el.classList.add('sel');}
async function doWd(){
  const amt=parseFloat(document.getElementById('wd-amt').value);const addr=document.getElementById('wd-addr').value.trim();
  if(!amt||amt<25){toast('Minimum $25','e');return;}if(!selCrypto){toast('Choisissez une crypto','e');return;}if(!addr){toast('Entrez votre adresse','e');return;}
  if(amt>(ME.balance||0)){toast('Solde insuffisant','e');return;}
  try{await api('POST','/api/withdrawals',{amount:amt,crypto:selCrypto,address:addr});toast('Demande envoyée !','s');selCrypto=null;goPage('aff-pay');}catch(e){toast(e.message,'e');}
}
...
async function doAddConv(){
  ...
  try{await api('POST','/api/conversions/manual',{user_id,offer_id,amount,status});closeMo();toast('Conversion ajoutée ✓','s');goPage('admin-conv');}catch(e){toast(e.message,'e');}
}
...
async function openCustomRequest(offerId,offerName){
  const requests=await api('GET','/api/custom-requests');
  const existing=requests.find(r=>r.offer_id===offerId);const v=existing||{};
    ...
    <div style="max-height:65vh;overflow-y:auto;padding-right:4px">
      ...
      <div class="fi"><label>🖼️ Logo du serveur</label>
        <div style="border:2px dashed #333;border-radius:10px;padding:14px;text-align:center;cursor:pointer" onclick="document.getElementById('cr-logo-file').click()">
          <input type="file" id="cr-logo-file" accept="image/*" style="display:none" onchange="previewCRFile(this,'cr-logo-prev')">
      ...
      <div class="fi">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:8px">
          ${[1,2,3,4,5,6].map(i=>'<div style="border:2px dashed #333;border-radius:10px;padding:10px;text-align:center;cursor:pointer" onclick="document.getElementById(\'cr-photo'+i+'\').click()"><input type="file" id="cr-photo'+i+'" accept="image/*" style="display:none" onchange="previewCRFile(this,\'cr-photo'+i+'-prev\')"><div id="cr-photo'+i+'-prev">'+(v['photo'+i+'_url']?'<img src="'+v['photo'+i+'_url']+'" style="width:100%;height:60px;object-fit:cover;border-radius:6px">':'<div style="font-size:20px">📷</div><div style="font-size:10px;color:var(--muted)">Photo '+i+'</div>')+'</div></div>').join('')}
        </div>
      </div>
...
function previewCRFile(input,previewId){const file=input.files[0];if(!file)return;const reader=new FileReader();reader.onload=e=>{const prev=document.getElementById(previewId);if(prev)prev.innerHTML='<img src="'+e.target.result+'" style="width:100%;max-height:80px;object-fit:cover;border-radius:6px">';};reader.readAsDataURL(file);}
async function submitCustomRequest(offerId){
  toast('Envoi en cours...','i');
  try{
    let logoUrl=null;const lf=document.getElementById('cr-logo-file').files[0];if(lf)logoUrl=await uploadImg(lf,'tickets');
    const photoUrls={};
    for(let i=1;i<=6;i++){const f=document.getElementById('cr-photo'+i).files[0];if(f)photoUrls['photo'+i+'_url']=await uploadImg(f,'tickets');}
    await api('POST','/api/custom-requests',{offer_id:offerId,server_name:document.getElementById('cr-name').value.trim(),slogan:document.getElementById('cr-slogan').value.trim(),tag1:document.getElementById('cr-tag1').value.trim(),tag2:document.getElementById('cr-tag2').value.trim(),tag3:document.getElementById('cr-tag3').value.trim(),logo_url:logoUrl,salons:document.getElementById('cr-salons').value.trim(),photo_text:document.getElementById('cr-phototext').value.trim(),photos_blurred:document.getElementById('cr-blur').checked,...photoUrls});
    closeMo();toast('Demande envoyée ✓','s');goPage('aff-links');
  }catch(e){toast(e.message,'e');}
}
...
// ── INIT ──
if(TOKEN&&ME){startApp();}else{showLanding();}
